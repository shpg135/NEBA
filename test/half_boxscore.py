...
def convert_box(file):
    ...
    #print (dog)
    awayFirstTable = soup.find('table',id ='box-{}-h1-basic'.format(awayTeam)).find_all('tr')
    awaySecondTable = soup.find('table',id ='box-{}-h2-basic'.format(awayTeam)).find_all('tr')
    homeFirstTable = soup.find('table',id ='box-{}-h1-basic'.format(homeTeam)).find_all('tr')
    homeSecondTable = soup.find('table',id ='box-{}-h2-basic'.format(homeTeam)).find_all('tr')

    for tr in awayFirstTable:
        quarter = 'h1'
        try:
        ...
        try:
            ...
            result.append([file,dog,quarter,awayTeam,homeFinScore,awayFinScore,winningTeam,losingTeam,belong_team,player,mp_convert,fg,fga,fg_pct,fg3,fg3a,fg3_pct,ft,fta,ft_pct,orb,drb,trb,ast,stl,blk,tov,pf,pts,plus_minus])
        except:
            pass


        for tr in homeFirstTable:
            quarter = 'h1'
            try:
                th_label = tr.find('th', {'class', 'left'})
                hyperlink = th_label.find('a')
            ...
            try:
                ...
                result.append(
                    [file, dog, quarter, awayTeam, homeFinScore, awayFinScore, winningTeam, losingTeam, belong_team,
...
if os.stat('{}{}_boxscore.csv'.format(season_folder,season)).st_size == 0:
    y.write('URL,GameDate,Quarter,HomeTeam,AwayTeam,HomeScore,AwayScore,WinningTeam,LosingTeam,Belong_Team,Player,Minute,FG,FGA,FG%,FG3,FG3A,FG3%,FT,FTA,FT%,ORB,DRB,TRB,AST,STL,BLK,TOV,PF,PTS,PLUMUS')
