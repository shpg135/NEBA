...
#season_folder = f"H:\\NEBA_new\\shotchart\\season\\{start_date[:4]}-{int(start_date[:4])+1}"
#os.makedirs(season_folder, exist_ok=True)
for year in year_list:
    season_folder = f"H:\\NEBA_new\\shotchart\\season\\{year}-{int(year) + 1}"
    os.makedirs(season_folder, exist_ok=True)
# Loop through all the files in the current directory
    for team in team_links:
        for file_name in os.listdir(r"H:\NEBA_new\shotchart\{}".format(team)):
    ...
    # Check if the date is within the season range
        ...
        # Move the file to the season subfolder
                shutil.move(os.path.join("H:\\NEBA_new\\shotchart\\{}\\".format(team),file_name), os.path.join(season_folder, file_name))